...
def get_dataset_path():
    ...
    csv_path = os.path.join(
        current_dir, "..", "..", "..", "data", "customer_reviews.csv"
    )
    return csv_path
        ...
        if "df" in st.session_state:
            try:
                with st.spinner("Analyzing sentiment..."):
                    st.session_state["df"].loc[:, "Sentiment"] = st.session_state["df"][
                        "SUMMARY"
                    ].apply(get_sentiment)
...
if "df" in st.session_state:
    ...
    if "Sentiment" in st.session_state["df"].columns:
        ...
        filtered_colors = {
            s: sentiment_colors[s] for s in existing_sentiments if s in sentiment_colors
        }

        # Reorder the data according to our custom order (only for existing sentiments)
        sentiment_counts["Sentiment"] = pd.Categorical(
            sentiment_counts["Sentiment"], categories=filtered_order, ordered=True
        )
