...
def _plot_dist_from_sma(ax, df):
    ax.plot(
        df.index, df["SMA20 %_mean"], label="Mean % vs SMA20", marker="o", markersize=3
    )
    ax.plot(
        df.index, df["SMA50 %_mean"], label="Mean % vs SMA50", marker="s", markersize=3
    )
    ax.plot(
        df.index,
        df["SMA200 %_mean"],
        label="Mean % vs SMA200",
        marker="^",
        markersize=3,
    )
    ax.axhline(0, color="grey", linestyle="--", linewidth=0.8)
...
def _plot_volume_and_volatility(ax, df):
    ...
    ax2.plot(
        df.index,
        df["Volatility W %_mean"],
        label="Mean Weekly Volatility %",
        marker="s",
        markersize=3,
        color=color,
    )
...
def plot_single_timeseries(
    series: pd.Series, title: str, ylabel: str, y_formatter: callable = None
):
    """
...
def plot_rank_with_criteria(
    ...
    fig.update_yaxes(
        autorange="reversed", dtick=100, showgrid=True, gridcolor="LightGrey"
    )
    ...
    if (
        filter_criteria
        and "recent_days" in filter_criteria
        and "lookback_days" in filter_criteria
    ):
        ...
        if len(all_dates) >= (lookback_days + recent_days):
            ...
            fig.add_vrect(
                x0=lookback_period_start_date,
                x1=lookback_period_end_date,
                fillcolor="LightGreen",
                opacity=0.2,
                layer="below",
                line_width=0,
                annotation_text="Lookback",
                annotation_position="top left",
            )
            fig.add_vline(
                x=recent_period_start_date,
                line_width=2,
                line_dash="dash",
                line_color="grey",
            )
        ...
        for sym in all_symbols:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    hovertemplate="%{y:.4f}<br>%{x|%Y-%m-%d}<extra>"
                    + name
                    + "</extra>",
                )
            )
