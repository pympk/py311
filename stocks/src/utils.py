...
def filter_df_dates_to_reference_symbol_obsolete_1(df, reference_symbol="AAPL"):
    ...
    if df.index.names[0] == "Symbol":
        ...
        print(
            f"Error: The first level of the DataFrame's index must be named 'Symbol' or 'Ticker', but found '{df.index.names[0]}'."
        )
        return pd.DataFrame()
    ...
    try:
        ...
        reference_dates = df.loc[reference_symbol].index
        if isinstance(
            reference_dates, pd.MultiIndex
        ):  # if reference_symbol still returns a multiindex (e.g. only one date)
            reference_dates = reference_dates.get_level_values("Date")

    except KeyError:
        print(
            f"Error: Reference symbol '{reference_symbol}' not found in DataFrame under index level '{symbol_level_name}'."
        )
        return pd.DataFrame()
    except (
        AttributeError
    ):  # Handles cases where .index might not behave as expected if loc result is not a DataFrame
        print(
            f"Error: Could not retrieve date index for reference symbol '{reference_symbol}'."
        )
        return pd.DataFrame()
    ...
    for symbol_val in original_symbols:
        try:  # Handle the case where a symbol might be missing or have issues
            ...
            if isinstance(
                symbol_dates, pd.MultiIndex
            ):  # if symbol_val still returns a multiindex
                symbol_dates = symbol_dates.get_level_values("Date")

        except KeyError:
            print(
                f"Warning: Symbol '{symbol_val}' caused a KeyError during date extraction. Skipping."
            )
            continue  # Skip to the next symbol if this one is missing or causes issues
        except (
            AttributeError
        ):  # Handles cases where .index might not behave as expected
            print(
                f"Warning: Could not retrieve date index for symbol '{symbol_val}'. Skipping."
            )
            continue
    ...
    if filtered_symbols:
        ...
        df_filtered = (
            pd.DataFrame()
        )  # Create an empty DataFrame with same columns if no symbols match
        if not df.empty:
        ...
        if isinstance(df.index, pd.MultiIndex):
            ...
            if (
                not df_filtered.index.empty
            ):  # if set_index resulted in non-empty (e.g. from existing columns)
    ...
    print(
        f"Number of {symbol_level_name}s filtered out: {len(original_symbols) - len(filtered_symbols)}"
    )
...
def filter_df_dates_to_reference_symbol(
    df: pd.DataFrame, reference_symbol: str
    ...
    ), "Input must be a DataFrame with a MultiIndex."
    assert df.index.names == [
        "Ticker",
        "Date",
    ], "Input DataFrame must have a ('Ticker', 'Date') MultiIndex."
    ...
    assert isinstance(
        df_filtered.index, pd.MultiIndex
    ), "Function failed to preserve the MultiIndex."
    assert df_filtered.index.names == [
        "Ticker",
        "Date",
    ], "Function failed to preserve the ('Ticker', 'Date') MultiIndex."
...
def filter_symbols_with_missing_values(
    ...
    # --- Post-condition Assertions: Guarantee Output ---
    assert isinstance(
        df_clean.index, pd.MultiIndex
    ), "Function failed to preserve the MultiIndex."
    assert df_clean.index.names == [
        "Ticker",
        "Date",
    ], "Function failed to preserve the ('Ticker', 'Date') MultiIndex."
...
def get_recent_files_in_directory_obsolete(
    ...
    try:
        ...
        )  # Debugging line to show the target directory
        ...
        candidate_files = [f for f in target_dir.glob(pattern) if f.is_file()]
...
def select_short_term_stocks_debug(
    ...
    inv_vol_col_name=DEFAULT_INV_VOL_COL_ST,
) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:  # Updated return type hint
    """
    ...
    if not isinstance(filters, dict) or not isinstance(scoring_weights, dict):
        ...
        try:
            ...
            pass  # Ignore errors during parameter gathering on failure
        return pd.DataFrame(), pd.DataFrame(), parameters_used

    if abs(sum(scoring_weights.values()) - 1.0) > EPSILON:
        logging.warning(
            f"Scoring weights provided do not sum to 1.0 (Sum={sum(scoring_weights.values())}). Proceeding, but normalization might be affected."
        )
    ...
    if missing_cols:
        logging.error(
            f"Missing required columns in df_finviz: {missing_cols}. Required based on filters/weights/inv_vol_col: {required_cols}"
        )
        parameters_used = {
            "n_select_requested": initial_n_select,
            "inv_vol_col_name": inv_vol_col_name,
    ...
    logging.info(
        f"Cleaned data: Removed {initial_count - cleaned_count} rows with NaNs in essential columns ({required_cols}). {cleaned_count} remaining."
    )
    if cleaned_count == 0:
    ...
    try:
        ...
        logging.error(
            f"Filtering failed. Check keys. Missing column/key: {e}. Filters: {filters}"
        )
        parameters_used = {
            "n_select_requested": initial_n_select,
            "inv_vol_col_name": inv_vol_col_name,
    ...
    if filtered_count == 0:
        ...
        return (
            ...
            df_after_filter,
            parameters_used,
        )  # Return empty selection, filtered df, params
    elif filtered_count < n_select:
        logging.warning(
            f"Only {filtered_count} stocks passed filters, less than n_select ({n_select}). Selecting all {filtered_count}."
        )
    ...
    try:
        ...
        logging.error(
            f"Scoring failed. Missing column/key: {e}. Weights: {scoring_weights}"
        )
        parameters_used = {
            "n_select_requested": initial_n_select,
            "inv_vol_col_name": inv_vol_col_name,
    ...
    logging.info(
        f"Applying 'EqualWeight', 'InverseVolatility', and 'ScoreWeighted' schemes..."
    )
    # -- Equal Weight (EW) --
    ...
    if not inv_vol_col_name or inv_vol_col_name not in df_selected.columns:
        ...
        if total_inv_vol > EPSILON:
            ...
            df_selected["Weight_IV"] = df_selected["Weight_EW"]  # Fallback
    # -- Score Weighted (SW) --
    scores = pd.to_numeric(df_selected["final_score"], errors="coerce").fillna(0)
    min_score = scores.min()
    if min_score < 0:
        scores = scores - min_score  # Shift to non-negative
    total_score = scores.sum()
    if abs(total_score) > EPSILON:
        df_selected["Weight_SW"] = scores / total_score
    else:
        df_selected["Weight_SW"] = df_selected["Weight_EW"]  # Fallback
    ...
    for w_col in weight_cols:
        ...
        if w_col in df_selected.columns and pd.api.types.is_numeric_dtype(
            df_selected[w_col]
        ):
            ...
            if pd.isna(weight_sum):
                ...
                logging.warning(
                    f"Final weights for '{w_col}' do not sum close to 1.0 (Sum = {weight_sum:.6f}). Check calculations."
                )
        elif w_col in df_selected.columns:
            logging.warning(f"Weight column '{w_col}' exists but is not numeric.")

    logging.info(
        f"Selected Stocks ({actual_n_select} stocks) with Scores and Weights (Top {min(5, actual_n_select)} shown):\n{df_selected.head(min(5, actual_n_select))}"
    )
...
def save_selection_results(
    ...
    try:
        if not df_selected.empty:
            df_selected.to_parquet(
                parquet_file, engine="pyarrow", compression="zstd", index=True
            )
            logging.info(
                f"Successfully saved selected stocks DataFrame to Parquet: {parquet_file}"
            )
        else:
            logging.warning(
                f"Selected stocks DataFrame is empty. Parquet file not saved: {parquet_file}"
            )
    ...
    if save_csv:
        try:
            if not df_selected.empty:
                df_selected.to_csv(
                    csv_file, index=True
                )  # IMPORTANT: Save the index (Tickers)
                logging.info(
                    f"Successfully saved selected stocks DataFrame to CSV: {csv_file}"
                )
            else:
                logging.warning(
                    f"Selected stocks DataFrame is empty. CSV file not saved: {csv_file}"
                )
        except Exception as e:
    ...
    try:
        ...
        logging.error(
            f"Failed to serialize parameters to JSON ({json_file}). Check for non-serializable types: {e}"
        )
        success = False
    except Exception as e:
...
def load_selection_results(
    ...
    if os.path.exists(parquet_file):
        try:
            ...
            logging.error(
                f"Failed to load DataFrame from Parquet ({parquet_file}): {e}"
            )
    ...
    if df_selected is None and os.path.exists(csv_file):
        logging.info(
            f"Parquet file ({parquet_file}) not found or failed to load. Attempting to load from CSV: {csv_file}"
        )
        try:
        ...
        logging.warning(
            f"Could not find Parquet ({parquet_file}) or CSV ({csv_file}) for DataFrame."
        )
    ...
    if df_selected is None and parameters_used is None:
        logging.error(
            f"Failed to load both DataFrame and Parameters for base path: {base_filepath}"
        )
...
def add_columns_from_source(
    ...
    if match_on_base_index and match_col_base is not None:
        raise ValueError(
            "Cannot specify both 'match_col_base' and 'match_on_base_index=True'. Choose one matching method."
        )
    if not match_on_base_index and match_col_base is None:
        raise ValueError(
            "Must specify 'match_col_base' if 'match_on_base_index' is False."
        )

    if not match_on_base_index:
        if match_col_base not in base_df.columns:
            raise KeyError(
                f"Matching column '{match_col_base}' not found in base_df columns: {base_df.columns.tolist()}"
            )
    ...
    if missing_source_cols:
        raise KeyError(
            f"Columns to add {missing_source_cols} not found in source_df columns: {source_df.columns.tolist()}"
        )
    ...
    # Execute the merge
    merged_df = pd.merge(base_df, **merge_kwargs)
    ...
    added_cols_actual_names = [
        col for col in all_merged_cols if col not in original_base_columns
    ]
    ...
    for col_name in added_cols_actual_names:
        ...
        if col_name in final_df.columns:
            ...
            if num_nas > 0:
                print(
                    f"Warning: {num_nas}/{total_rows} entries for added column '{col_name}' are NaN."
                )
                if num_nas == total_rows:
                    print(
                        f"   -> Check if values in the base_df {match_key_description} exist in the index of the source DataFrame."
                    )
        else:
            # This case should ideally not happen with the reindex approach
            print(
                f"Warning: Expected added column '{col_name}' was not found in the final reordered DataFrame."
            )
...
def z_score_series(series: pd.Series) -> pd.Series:
    ...
    ):  # Handle case where all values are NaN after coercion
        return pd.Series(np.nan, index=series.index).rename(f"z_{series.name}")
    return pd.Series(
        zscore(numeric_series, nan_policy="omit"), index=series.index
    ).rename(f"z_{series.name}")
...
def load_price_data(price_file_path: Path) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        logging.info(
            f"Successfully loaded and prepared price data. Shape: {df_prices.shape}"
        )
        return df_prices
    except Exception as e:
...
def find_and_pair_selection_files(selection_dir: Path) -> List[Tuple[Path, Path]]:
    ...
    for data_file in sorted(all_parquet_files):
        ...
        if date_key in param_map:
            ...
            logging.warning(
                f"No matching parameter file for data file: {data_file.name}"
            )
...
def run_single_backtest(
    ...
    try:
        ...
        try:
            ...
            indexer = all_trading_dates.get_indexer(
                [selection_timestamp], method="ffill"
            )
            if indexer[0] == -1:  # Date is before the first date in the index
                logging.error(
                    f"  Error: Selection date {selection_date} is before any available price data."
                )
                return None
            ...
            logging.warning(
                f"  Warning: Exact selection date {selection_date} not found. Using previous available date: {actual_selection_date_used.strftime('%Y-%m-%d')}"
            )

        if selection_idx + 1 >= len(all_trading_dates):
            logging.error(
                f"  Error: No trading date found after selection date ({actual_selection_date_used.strftime('%Y-%m-%d')}). Cannot determine buy date."
            )
            return None
        ...
        if selection_idx + 2 >= len(all_trading_dates):
            logging.error(
                f"  Error: No trading date found after buy date ({buy_date.strftime('%Y-%m-%d')}). Cannot determine sell date."
            )
            return None
        sell_date = all_trading_dates[selection_idx + 2]

        logging.info(
            f"  Selection Date Used: {actual_selection_date_used.strftime('%Y-%m-%d')}"
        )
        ...
        for ticker, weight in ticker_weights.items():
            ...
            if pd.isna(buy_price) or buy_price <= 0 or pd.isna(sell_price):
                logging.warning(
                    f"    - Invalid price data for {ticker} (Buy: {buy_price}, Sell: {sell_price}). Skipping trade."
                )
                continue
        ...
        metrics = {
            ...
            "portfolio_return_normalized": (
                portfolio_return / total_weight_traded
                if abs(total_weight_traded) > 1e-9
                else 0.0
            ),
            ...
            "std_dev_return": np.nan,
            "sharpe_ratio_period": np.nan,
        }
        ...
        if num_successful_trades > 0:
            ...
            metrics["std_dev_return"] = (
                np.std(returns_array, ddof=1) if num_successful_trades > 1 else 0.0
            )
            ...
            if std_dev > 1e-9:
                ...
                metrics["sharpe_ratio_period"] = (
                    np.inf * np.sign(excess_return)
                    if abs(excess_return) > 1e-9
                    else 0.0
                )

            logging.info(
                f"  Trades Executed: {num_successful_trades}/{len(ticker_weights)}"
            )
            logging.info(f"  Portfolio Return : {metrics['portfolio_return']:.4f}")
            logging.info(f"  Win Rate         : {metrics['win_rate']:.2%}")
        else:
            logging.warning(
                f"  No successful trades executed out of {len(ticker_weights)} attempted."
            )

        backtest_results = {
            "run_inputs": {
                "selection_date": selection_date,
                "actual_selection_date_used": actual_selection_date_used.strftime(
                    "%Y-%m-%d"
                ),
        ...
        logging.info(
            f"Backtest simulation for '{scheme_name}' on {selection_date} completed."
        )
        return backtest_results

    except Exception as e:
        logging.critical(
            f"  FATAL ERROR during backtest run for {selection_date}, {scheme_name}: {e}",
            exc_info=True,
        )
        return None
...
def update_and_save_results_obsolete(
    new_records: List[Dict[str, Any]], results_path: Path, unique_key_cols: List[str]
):
    ...
    for key in unique_key_cols:
        if key not in df_combined.columns:
            logging.warning(
                f"Unique key '{key}' not in DataFrame. Skipping deduplication on this key."
            )
            continue

    # Final sort for consistent output
    df_final = df_combined.sort_values("run_timestamp").drop_duplicates(
        subset=unique_key_cols, keep="last"
    )
    df_final = df_final.sort_values(
        by=["selection_date", "scheme"], ascending=[False, True]
    )
    ...
    logging.info(
        f"Successfully saved {len(df_final)} consolidated records to {results_path.name}"
    )
...
def update_and_save_results(
    ...
    df_new["run_timestamp"] = pd.to_datetime(
        df_new.get("run_timestamp", pd.Timestamp.now())
    )
    ...
    date_key_present = any(
        col in ["selection_date", "actual_selection_date_used"]
    ...
    try:
        ...
        for weight_col in weight_cols:
            ...
            if not ticker_weights:
                logging.warning(
                    f"No tickers for scheme '{scheme_name}' on {selection_date}. Skipping."
                )
                continue
            ...
            if result:
                ...
                record = {
                    ...
                    "selection_date": selection_date,
                    "actual_selection_date_used": result.get("run_inputs", {}).get(
        ...
        logging.error(
            f"Failed to process pair {data_file.name}/{param_file.name}: {e}",
            exc_info=True,
        )
...
def plot_cumulative_returns(
    ...
    for strategy in top_strategies:
        ...
        mask = (df[strategy_id_cols] == strategy_params).all(axis=1) & (
            df[scheme_col] == scheme
        )
        ...
        if not subset.empty:
            ...
            params_str = ", ".join(
                [
                    f"{col.split('_')[-1]}={val}"
                    for col, val in zip(strategy_id_cols, strategy_params)
                ]
            )
    ...
    ax.set_title(
        f"Equity Curve for Top {top_n_strategies} Strategy Run(s)", fontsize=16
    )
...
def plot_evolving_annualized_sharpe(
    ...
    if not all(col in df.columns for col in [date_col, return_col, scheme_col]):
        raise ValueError(
            f"DataFrame must contain columns: {date_col}, {return_col}, {scheme_col}"
        )
    ...
    if not isinstance(min_periods_for_sharpe, int) or min_periods_for_sharpe < 2:
        raise ValueError(
            "min_periods_for_sharpe must be an integer greater than or equal to 2."
        )
    ...
    for scheme_name, group in df_analysis.groupby(scheme_col):
        group["expanding_mean_return"] = (
            group[return_col].expanding(min_periods=min_periods_for_sharpe).mean()
        )
        group["expanding_std_return"] = (
            group[return_col].expanding(min_periods=min_periods_for_sharpe).std()
        )
        ...
        group["annualized_sharpe_ratio"] = group["daily_sharpe_ratio"] * np.sqrt(
    ...
    for scheme in df_results[scheme_col].unique():
        subset_plot = df_results[df_results[scheme_col] == scheme].dropna(
            subset=["annualized_sharpe_ratio", date_col]
        )
        if not subset_plot.empty:
            ax.plot(
                subset_plot[date_col],
                subset_plot["annualized_sharpe_ratio"],
    ...
    ax.set_title(
        f"Evolving Annualized Sharpe Ratio (vs. {annual_risk_free_rate*100:.1f}% Ann. Risk-Free Rate)",
        fontsize=16,
    )
    ax.set_xlabel(f"Date ({date_col})", fontsize=12)
    ax.set_ylabel(
        f"Annualized Sharpe Ratio (using {trading_days_per_year} days/year)",
        fontsize=12,
    )
    ...
    if all_plotted_dates_objects:
        unique_plotted_dates = sorted(
            list(pd.Series(all_plotted_dates_objects).unique())
        )
        unique_plotted_dates = [
            pd.Timestamp(d) for d in unique_plotted_dates if pd.notna(d)
        ]
...
def calculate_rank_metrics_obsolete(
    df_rank_history, tickers_list, lookback_days=20, recent_days=4
):
    """
    ...
    if len(df_rank_history.columns) < total_days_needed:
        print(
            f"Error: Not enough data. Need {total_days_needed} days, have {len(df_rank_history.columns)}."
        )
        return []
    ...
    lookback_dates = df_rank_history.loc[
        :, lookback_period_start_date:lookback_period_end_date
    ].columns
    ...
    for ticker in tickers_list:
        ...
        total_peak_rank = int(
            all_ranks_in_period.min()
        )  # Best rank over the whole period
        ...
        metrics_dict = {
            ...
            "recent_bottom_to_recent_start": recent_bottom_rank
            - rank_at_start_of_recent,
...
def filter_rank_metrics(
    ...
    if min_current_to_recent_start is not None and (
        min_recent_bottom_to_recent_start is not None
        or min_recent_bottom_to_current is not None
    ):
        raise ValueError(
            "Cannot specify parameters for both 'Dip' and 'Reversal' modes."
        )
    ...
    if min_current_to_recent_start is not None:
        ...
        min_recent_bottom_to_recent_start is not None
        and min_recent_bottom_to_current is not None
    ):
        # --- Reversal Mode ---
        print("Filtering in 'Reversal' mode...")
        df = df[
            df["recent_bottom_to_recent_start"] >= min_recent_bottom_to_recent_start
        ]
        df = df[df["recent_bottom_to_current"] >= min_recent_bottom_to_current]
    else:
        print(
            "Warning: No mode-specific filters applied. Returning candidates based on general filters only."
        )
...
def create_rolling_window_chunks(
    returns_df: pd.DataFrame, window_width: int, step_size: int
) -> List[pd.DataFrame]:
    """
    ...
    if num_rows < window_width:
        print(
            f"Warning: Data length ({num_rows}) is less than window width ({window_width}). No chunks created."
        )
        return []
...
def align_weekend_data_files_to_fridays(
    search_dir: Path,
    prefix: str,
    file_ext: str,
):
    ...
    for weekend_file_path in all_files:
        ...
        if file_date.weekday() >= 5:
            ...
            if target_friday_path.exists():
                print(
                    f"  -> Target file already exists. It will be overwritten to ensure the latest data is used."
                )
                target_friday_path.unlink()
            else:
                print(
                    f"  -> Target file does not exists. Created Friday file from weekend file."
                )
...
def filter_symbols_with_extreme_changes(
    ...
    tickers_to_remove = (
        df[extreme_change_mask].index.get_level_values("Ticker").unique().tolist()
    )
...
def trim_dataframe_to_recent_days(df: pd.DataFrame, days_to_keep: int) -> pd.DataFrame:
    """
