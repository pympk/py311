...
class QuantUtils:
    ...
    #     """
    #     # 1. Get shifted versions of the data
    #     # (Since we are inside TickerEngine.map_kernels, 'series' is a single ticker)
    #     s0 = series
    #     s1 = series.shift(1)
    #     s2 = series.shift(2)
    #     s3 = series.shift(3)
    ...
    #     # 2. Weighted Sum (The OLS numerator for n=5, centered x=[-2,-1,0,1,2])
    #     # Denominator (sum of x^2) for this centered x is 10.
    #     return (2 * s0 + 1 * s1 + 0 * s2 - 1 * s3 - 2 * s4) / 10.0

    # @staticmethod
    # def calculate_obv_fast(df: pd.DataFrame) -> pd.Series:
    #     """Vectorized OBV for the whole dataframe at once."""
    #     close = df["Adj Close"]
    #     vol = df["Volume"]

    #     # Direction: 1 if up, -1 if down, 0 if flat
    #     # We do this globally, then mask the first entry per ticker
    #     direction = np.sign(close.groupby(level="Ticker").diff().fillna(0))
    #     obv = (direction * vol).groupby(level="Ticker").cumsum()
    #     return obv
