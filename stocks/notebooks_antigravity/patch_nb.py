...
if target_cell:
    ...
    for line in lines:
        ...
        if "fig.update_layout(" in line and "hovermode='x unified'" in line:
            new_line = line.replace("hovermode='x unified'", "hovermode='x unified', autosize=True, margin=dict(l=20, r=20, t=40, b=20)")
        ...
        # Fix 2: Convert indices to pydatetime
            ...
            new_line = line.replace("x=res.normalized_plot_data.index", "x=res.normalized_plot_data.index.to_pydatetime()")
            new_source.append(new_line)
            
        elif "fig.data[50].update(x=res.benchmark_series.index" in line:
            new_line = line.replace("x=res.benchmark_series.index", "x=res.benchmark_series.index.to_pydatetime()")
            new_source.append(new_line)
            
        elif "fig.data[51].update(x=res.portfolio_series.index" in line:
            new_line = line.replace("x=res.portfolio_series.index", "x=res.portfolio_series.index.to_pydatetime()")
            new_source.append(new_line)
